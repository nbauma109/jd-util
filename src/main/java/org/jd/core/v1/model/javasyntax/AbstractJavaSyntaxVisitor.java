...
public abstract class AbstractJavaSyntaxVisitor extends AbstractTypeArgumentVisitor implements DeclarationVisitor, ExpressionVisitor, ReferenceVisitor, StatementVisitor, TypeVisitor, TypeParameterVisitor {
    ...
    public void visit(SwitchExpression expression) {
        ...
        for (SwitchExpression.Rule rule : expression.getRules()) {
            rule.accept(this);
        }
    }
    ...
    @Override
    public void visit(SwitchExpression.ExpressionLabel label) {
        label.getExpression().accept(this);
    }
    ...
    @Override
    public void visit(SwitchExpression.RuleStatement rule) {
        for (SwitchExpression.Label label : rule.getLabels()) {
            label.accept(this);
        }
        safeAccept(rule.getWhenCondition());
