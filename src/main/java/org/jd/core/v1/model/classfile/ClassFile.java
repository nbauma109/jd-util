...
public class ClassFile {
	...
	public String[] getInterfaceTypeNames() {
		...
		for (int i = 0; i < interfaceNames.length; i++) {
			interfaceNames[i] = this.getConstantPool().getConstantString(interfaceIndices[i], CONSTANT_Class);
		}
		return interfaceNames;
	}

	public String getSuperTypeName() {
		return this.getConstantPool().getConstantString(this.getSuperclassNameIndex(), CONSTANT_Class);
	}
	...
	public String toString() {
		return "ClassFile{" + this.getInternalTypeName() + "}"; //$NON-NLS-1$ //$NON-NLS-2$
	}
}
