...
public class FieldReferenceExpression extends AbstractLineNumberTypeExpression {
	...
	public String toString() {
		return "FieldReferenceExpression{type=" + this.getType() + ", expression=" + expression + ", name=" + name + ", descriptor=" + descriptor + "}"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
	}

	@Override
	public Expression copyTo(int lineNumber) {
		return new FieldReferenceExpression(lineNumber, this.getType(), expression, internalTypeName, name, descriptor);
	}
}
