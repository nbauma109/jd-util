...
public class TokenizeJavaFragmentVisitor implements JavaFragmentVisitor {
    ...
    protected static final KeywordToken STATIC = new KeywordToken("static");
    ...
    public void visit(ImportsFragment fragment) {
        List<JavaImport> imports = new DefaultList<>(fragment.getImports());

        imports.sort(Comparator.comparing(JavaImport::getQualifiedName));

        tokens.add(StartMarkerToken.IMPORT_STATEMENTS);

        for (JavaImport imp : imports) {
            tokens.add(IMPORT);
            tokens.add(TextToken.SPACE);
            if (imp.isStatic()) {
                tokens.add(STATIC);
